...
const (
	...
	// DreamPhaseUncoveredBackfill enqueues a full enrichment job for every live
	// memory that has never been enriched (durable memories.enriched = false:
	// enrichment disabled at creation, a failed write-time enqueue, a direct
	// import/migration), the unconditional safety net that replaces the removed
	// NRAM_ENABLE_ENRICHMENT_BACKFILL boot hook. Coverage is the durable enriched
	// flag, not enrichment_queue rows, so a completed memory is never re-enqueued.
	// Unlike the embedding-backfill phase (which only re-embeds), the full job
	// also restores extracted facts, entities, and relationships.
