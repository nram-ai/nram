...
func RegisterRecallTool(s *Server) {
	tool := mcp.NewTool("recall",
		...
		mcp.WithDescription("Search persistent memory. ALWAYS recall at the start of a new task to load context. Recall before making assumptions and before storing to avoid duplicates. Use natural language queries. For a multi-intent need, issue one focused, single-intent recall per intent rather than one keyword-stuffed query: packing several distinct topics into a single query blends the embedding and drops the weaker facets, so a broad grab-bag query returns the dominant topic and buries the rest. Specifying a project searches that project plus the reserved global (world-knowledge) and about_me (the user's self-knowledge) tiers; omitting a project searches global plus about_me. The about_me persona tier always joins the recall by association; use the about_me tool to load it directly. Graph entities and relationships are always included when the knowledge graph is populated. When results look noisy or miss a specific fact you expect to exist, do not just re-query with reworded prose: call the graph tool on the key concept you are after, follow its relationships to the entity that should hold the fact, and fetch the source memory behind that relationship (its source_memory id) with get."),
