...
func createTestUser(t *testing.T, ctx context.Context, db DB) *model.User {
	...
	var orgIDStr string
	orgQuery := "SELECT id FROM organizations WHERE namespace_id = ?"
	if db.Backend() == BackendPostgres {
		orgQuery = "SELECT id FROM organizations WHERE namespace_id = $1"
	}
	row := db.QueryRow(ctx, orgQuery, orgNS.ID.String())
