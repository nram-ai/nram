...
// TruncateAllVectors drops every memory and entity collection across all
// dimensions, then recreates them via EnsureCollections. Drop-and-recreate
// is the simplest correct path on Qdrant: it preserves the namespace_id
// payload index (which EnsureCollections re-creates) and avoids the cost
// of a points-by-filter delete that would have to walk every point.
//
// Used by the embedding-model switch cascade — see the cascade docs for
// why all-dim wipe is required (vectors generated by the prior model are
// invalid in the new model's vector space).
