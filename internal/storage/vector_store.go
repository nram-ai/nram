...
type VectorStore interface {
	...
	// TruncateAllVectors removes every vector across every dimension for
	// both memory and entity kinds. Used by the embedding-model switch
	// cascade: when the operator changes the embedder, all stored vectors
	// were generated by the old model and are now invalid (no cross-model
	// retrieval). Backends MUST clear all per-dim tables/collections, not
	// just the dim that was active. The schema is preserved (no DROP) so
	// re-population starts immediately.
