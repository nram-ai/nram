...
var settingsSchemas = []api.SettingSchema{
	...
	{Key: "dreaming.enabled", Type: "boolean", DefaultValue: json.RawMessage(`true`), Description: "Enable background dreaming (memory consolidation and graph improvement)", Category: "dreaming"},
	...
	{Key: service.SettingDreamAugmentationBackfillFraction, Type: "number", DefaultValue: json.RawMessage(`0.0`), Description: "Relative weight of the augmentation-backfill phase among LLM-spending phases (0.0-1.0). Enqueue-only phase; default 0.0 means no per-phase slice: it enqueues query-augmentation jobs and shares the root budget, issuing no LLM calls of its own.", Category: "dreaming_phase_budget", Min: ptrF(0), Max: ptrF(1), Step: ptrF(0.05)},
	...
	{Key: service.SettingDreamAugmentationBackfillEnabled, Type: "boolean", DefaultValue: json.RawMessage(`true`), Description: "Run the augmentation-backfill phase. Enqueues query-augmentation enrichment jobs for live memories whose vector was built from raw content (augmented_embedding_at IS NULL): e.g. dream syntheses or stores whose augmentation step was skipped while the augment provider was briefly unavailable. Automates the admin backfill-augmentation action so stranded rows self-heal each cycle.", Category: "dreaming_augmentation_backfill"},
	{Key: service.SettingDreamAugmentationBackfillCapPerCycle, Type: "number", DefaultValue: json.RawMessage(`1000`), Description: "Maximum un-augmented memories enqueued per dream cycle by the augmentation-backfill phase. Residual is signalled when more candidates remain than the cap allowed so the backlog drains across cycles.", Category: "dreaming_augmentation_backfill", Min: ptrF(0), Max: ptrF(100000), Step: ptrF(50)},
	...
	{Key: "reconsolidation.mode", Type: "enum", DefaultValue: json.RawMessage(`"persist"`), Description: "Reconsolidation mode: 'shadow' emits events without persisting; 'persist' writes confidence/access updates; 'off' disables reinforcement entirely", Category: "reconsolidation", EnumValues: []string{"shadow", "persist", "off"}},
	...
	{Key: "reconsolidation.decay_enabled", Type: "boolean", DefaultValue: json.RawMessage(`true`), Description: "Enable sleep-time confidence decay for memories not recalled recently", Category: "reconsolidation"},
	...
	{Key: "recall.fusion.enabled", Type: "boolean", DefaultValue: json.RawMessage(`true`), Description: "Enable hybrid recall: parallel vector + BM25/tsvector retrieval fused via Reciprocal Rank Fusion. Requires migration 18 applied and a lexical searcher wired; when the lexical searcher is not wired, recall falls back to vector-only, so enabling this is safe on any backend.", Category: "recall_fusion"},
	...
	{Key: service.SettingIngestionDecisionEnabled, Type: "boolean", DefaultValue: json.RawMessage(`true`), Description: "Enable LLM-driven ingestion decision (ADD/UPDATE/DELETE/NONE) on near-duplicate matches at enrichment time. When off, every memory is treated as ADD without an LLM call.", Category: "enrichment_ingestion"},
	{Key: service.SettingIngestionDecisionShadow, Type: "boolean", DefaultValue: json.RawMessage(`false`), Description: "Shadow mode: compute and log the decision (op, top_score, match_count) but always behave as if it were ADD. Off by default, so the judge acts on its UPDATE/DELETE decisions; set true to observe the decision distribution without superseding or deleting any memory.", Category: "enrichment_ingestion"},
	...
	{Key: service.SettingQueryAugmentEnabled, Type: "boolean", DefaultValue: json.RawMessage(`true`), Description: "Enable query augmentation: at enrichment time generate N paraphrased query forms per memory and prepend them to content before embedding. On by default. Recommended validation before relying on it: confirm the canned recall regression set shows no contamination-probe regressions plus measurable improvement on 3+ of 7 stress angles. Use the Backfill button below to re-embed memories stored before the flag was on.", Category: "enrichment_query_augment"},
	...
	{Key: service.SettingRankWeightOrigin, Type: "number", DefaultValue: json.RawMessage(`0.25`), Description: "Weight on the project-affinity term: candidates whose home namespace is the recall's primary project get this added to their score (0.0-1.0). Default 0.25 lifts on-topic project memories above the residual freshness edge that recently-written global/about_me (persona) memories carry on project-scoped queries, while still leaving genuine cross-tier hits (e.g. an identity question answered from about_me) on top. Derived from a live sweep against the production corpus; the measured safe window was [0.20, 0.35]. Operators tune per project via the ranking_weights override on project settings.", Category: "ranking", Min: ptrF(0), Max: ptrF(1), Step: ptrF(0.05)},
	...
	{Key: service.SettingEnrichmentFailedRetentionDays, Type: "number", DefaultValue: json.RawMessage(`7`), Description: "How many days a permanently-failed enrichment job is retained before the stuck-job sweeper hard-deletes it (by now() - updated_at). Keeps the failed backlog from growing without bound when a slow local provider accumulates large numbers of failed jobs. Set to 0 to disable pruning and keep failed rows forever. Hot-reloadable.", Category: "enrichment_performance", Min: ptrF(0), Max: ptrF(3650), Step: ptrF(1)},
	...
	{Key: service.SettingRecallGraphReserveFraction, Type: "number", DefaultValue: json.RawMessage(`0.15`), Description: "Fraction of the MCP recall byte budget (mcp.max_result_tokens) reserved for the knowledge graph. The recall tool packs a balanced, signal-sorted subset of entities and relationships into this slice before trimming memories, so recall always surfaces some graph context instead of dropping it wholesale. 0 disables the graph in recall results. Hot-reloadable.", Category: "recall", Min: ptrF(0), Max: ptrF(0.5), Step: ptrF(0.01)},
	{Key: service.SettingRecallGraphVectorActivationEnabled, Type: "boolean", DefaultValue: json.RawMessage(`true`), Description: "Cross-namespace vector-channel entity activation in recall. When on (default), recall activates entities by vector similarity across the [project, global] aperture and boosts their connected memories. Off reproduces primary-only lexical activation.", Category: "recall"},
	{Key: service.SettingRecallGraphVectorActivationTopK, Type: "number", DefaultValue: json.RawMessage(`5`), Description: "Entities fetched per namespace by the vector activation channel.", Category: "recall", Min: ptrF(1), Max: ptrF(50), Step: ptrF(1)},
	{Key: service.SettingRecallGraphMaxEdges, Type: "number", DefaultValue: json.RawMessage(`2000`), Description: "Per-recall graph traversal edge budget (distinct from graph.max_edges, which caps the visualization endpoint). Split fairly per seed.", Category: "recall", Min: ptrF(100), Max: ptrF(100000), Step: ptrF(100)},
