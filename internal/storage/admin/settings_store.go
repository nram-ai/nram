...
var settingsSchemas = []api.SettingSchema{
	...
	{Key: service.SettingAPIRateLimitRPS, Type: "number", DefaultValue: json.RawMessage(`10`), Description: "Sustained API rate limit, in requests per second per user.", Category: "api", RequiresRestart: true, Min: ptrF(1), Max: ptrF(10000), Step: ptrF(1)},
	{Key: service.SettingAPIRateLimitBurst, Type: "number", DefaultValue: json.RawMessage(`20`), Description: "How many requests a user may burst above the per-second limit before being throttled.", Category: "api", RequiresRestart: true, Min: ptrF(1), Max: ptrF(10000), Step: ptrF(1)},
	{Key: service.SettingAPIAuthRateLimitRPS, Type: "number", DefaultValue: json.RawMessage(`5`), Description: "Sustained rate limit, in requests per second per client IP, on the unauthenticated auth endpoints (password login, lookup, passkey begin/finish, IdP login) and the public OAuth token/register endpoints. Keyed on the client IP (CF-Connecting-IP, then X-Real-IP, then the first X-Forwarded-For hop when behind a proxy) since these callers have no user identity yet.", Category: "api", RequiresRestart: true, Min: ptrF(1), Max: ptrF(10000), Step: ptrF(1)},
	{Key: service.SettingAPIAuthRateLimitBurst, Type: "number", DefaultValue: json.RawMessage(`10`), Description: "How many requests a client IP may burst above the per-second pre-auth limit before being throttled with 429.", Category: "api", RequiresRestart: true, Min: ptrF(1), Max: ptrF(10000), Step: ptrF(1)},
	{Key: service.SettingServerHostRebindingProtection, Type: "boolean", DefaultValue: json.RawMessage(`false`), Description: "Reject requests that arrive over loopback but carry a non-loopback Host header, the shape a DNS rebinding attack takes against a server reachable at localhost. Leave this OFF if nram sits behind a reverse proxy on the same machine that forwards to 127.0.0.1 and preserves the public Host, which is the setup the quickstart recommends for TLS: turning it on rejects every such request with 403. Safe to turn on when clients reach nram directly, or when the proxy connects over a LAN or container address. Applies to the authenticated API and MCP surface.", Category: "transport"},
	{Key: service.SettingServerSecureCookies, Type: "boolean", DefaultValue: json.RawMessage(`false`), Description: "Force the Secure attribute on the short-lived nram_session cookie unconditionally, instead of deriving it per-request from TLS / X-Forwarded-Proto. Turn this ON in any production deployment served over HTTPS (directly or behind a TLS-terminating reverse proxy) so the session cookie is never sent over plaintext even if a proxy drops the X-Forwarded-Proto header. Leave OFF only for local http:// development, where a Secure cookie would not be sent at all. Applies to every login path (password, passkey, IdP, consent refresh).", Category: "transport", RequiresRestart: true},
	...
	// SQLite maintenance sweeper. AppliesToBackend scopes the UI to SQLite-only
	// deployments; on Postgres these have no effect (autovacuum handles it).
	{Key: service.SettingSqliteMaintEnabled, Type: "boolean", DefaultValue: json.RawMessage(`true`), Description: "Run the background SQLite maintenance sweeper, which reclaims free pages, refreshes query-planner statistics, and periodically compacts the database file. SQLite only.", Category: "sqlite_maintenance", AppliesToBackend: []string{storage.BackendSQLite}},
	{Key: service.SettingSqliteMaintLightIntervalSeconds, Type: "number", DefaultValue: json.RawMessage(`3600`), Description: "How often the light SQLite maintenance pass runs: incremental vacuum, PRAGMA optimize, and a WAL checkpoint. Cheap and non-blocking.", Category: "sqlite_maintenance", AppliesToBackend: []string{storage.BackendSQLite}, Min: ptrF(60), Max: ptrF(86400), Step: ptrF(60)},
	{Key: service.SettingSqliteMaintFullVacuumIntervalSeconds, Type: "number", DefaultValue: json.RawMessage(`604800`), Description: "How often a full VACUUM runs to compact the database file and return free space to the OS. A full VACUUM briefly locks the database and can degrade performance while it runs, so keep it infrequent.", Category: "sqlite_maintenance", AppliesToBackend: []string{storage.BackendSQLite}, Min: ptrF(3600), Max: ptrF(31536000), Step: ptrF(3600)},
	...
	{Key: service.SettingEnrichmentFailedMaxRows, Type: "number", DefaultValue: json.RawMessage(`10000`), Description: "Maximum permanently-failed enrichment jobs kept per namespace regardless of age, so a sudden burst of failures (a misconfigured provider) can't pile up faster than the age-based retention can drain it. The newest jobs are kept and older ones deleted. Set to 0 to disable the count cap.", Category: "enrichment_performance", Min: ptrF(0), Max: ptrF(10000000), Step: ptrF(1000)},
	...
	{Key: service.SettingRecallDecompositionEnabled, Type: "boolean", DefaultValue: json.RawMessage(`true`), Description: "Break a multi-facet recall query (\"my health, my finances, and my hardware preferences\") into one focused sub-query per facet, recall each, and interleave them so a dominant facet can't bury a requested one. A single dense query vector blends the facets and drops the weakest; decomposing recovers them. One small extra LLM call on the decomposition provider per decomposed recall; a no-op for short/single-facet queries and when no decomposition LLM slot is configured. Reuses the ask decomposition prompt and knobs.", Category: "recall"},
	{Key: service.SettingRecallDecompositionMinWords, Type: "number", DefaultValue: json.RawMessage(`6`), Description: "Hot-path guard for recall decomposition: a query with fewer words than this skips the decomposer LLM call and takes the single-vector path. Keeps default-on decomposition from taxing the common short, focused recall; only longer, plausibly multi-facet queries pay the extra call.", Category: "recall", Min: ptrF(1), Max: ptrF(100), Step: ptrF(1)},
	{Key: service.SettingRecallDecompositionFloorRatio, Type: "number", DefaultValue: json.RawMessage(`0.5`), Description: "When interleaving a decomposed recall's sub-query results, a candidate is kept only if its score is at least this fraction of its own sub-query's top hit. Floors each sub-query against its own cluster (never a shared global top) so a minority facet's on-topic hits aren't swamped by a dominant facet's higher scores. 0 disables the floor.", Category: "recall", Min: ptrF(0), Max: ptrF(1), Step: ptrF(0.05)},
	...
	{Key: service.SettingAskConfidenceCosineFloor, Type: "number", DefaultValue: json.RawMessage(`0.35`), Description: "Calibration floor for the score reported as an answer's confidence: a cited source's vector cosine at or below this reads as no grounding. That score measures how strongly the cited sources match the query, not whether the answer is right. Tune to your embedder's similarity band (qwen3-embedding strong matches cluster ~0.55-0.71).", Category: "ask", Min: ptrF(0), Max: ptrF(1), Step: ptrF(0.01)},
	{Key: service.SettingAskConfidenceCosineCeiling, Type: "number", DefaultValue: json.RawMessage(`0.75`), Description: "Calibration ceiling for the same score: a cited source's vector cosine at or above this reads as full grounding. Must be greater than the floor.", Category: "ask", Min: ptrF(0), Max: ptrF(1), Step: ptrF(0.01)},
	...
	{Key: service.SettingAskSynthesisStructuredEnabled, Type: "boolean", DefaultValue: json.RawMessage(`true`), Description: "Answer each part of a question in one structured (JSON) synthesis call, so a multi-part question whose absent part is asked first no longer makes the model decline the whole answer. The model splits the question into parts, answers each supported part from the neighborhood, and marks unsupported parts \"not specified in the memories\"; the answer is assembled deterministically. Falls back to the plain Answer Synthesis prompt on any provider or parse failure. On by default.", Category: "ask"},
...
var promptSchemaEntries = []api.SettingSchema{
	...
	{Key: service.SettingAskSynthesisStructuredSystemPrompt, Type: "prompt", Description: "Structured system prompt for the ask tool used when ask.synthesis.structured.enabled is on: split the question into distinct parts and return a JSON parts array, answering each supported part from the neighborhood with inline memory-id citations and marking unsupported parts \"not specified in the memories\". The plain Answer Synthesis prompt is the fallback.", Category: "ask_prompts"},
