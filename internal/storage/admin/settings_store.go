...
var settingsSchemas = []api.SettingSchema{
	...
	{Key: "dreaming.min_interval_seconds", Type: "number", DefaultValue: json.RawMessage(`3600`), Description: "Minimum seconds between dream cycles for the same project", Category: "dreaming", Min: ptrF(60), Max: ptrF(2592000), Step: ptrF(300)},
	...
	{Key: "dreaming.consolidation.audit_budget_fraction", Type: "number", DefaultValue: json.RawMessage(`0.35`), Description: "Fraction of remaining cycle budget reserved for the novelty audit sub-phase (0.0-1.0)", Category: "dreaming_consolidation", Min: ptrF(0), Max: ptrF(1), Step: ptrF(0.05)},
	{Key: "dreaming.consolidation.reinforce_budget_fraction", Type: "number", DefaultValue: json.RawMessage(`0.35`), Description: "Fraction of remaining cycle budget reserved for the reinforcement sub-phase (0.0-1.0)", Category: "dreaming_consolidation", Min: ptrF(0), Max: ptrF(1), Step: ptrF(0.05)},
	{Key: "dreaming.consolidation.consolidate_budget_fraction", Type: "number", DefaultValue: json.RawMessage(`0.30`), Description: "Fraction of remaining cycle budget reserved for the consolidation sub-phase (0.0-1.0)", Category: "dreaming_consolidation", Min: ptrF(0), Max: ptrF(1), Step: ptrF(0.05)},
	{Key: service.SettingDreamEntityDedupFraction, Type: "number", DefaultValue: json.RawMessage(`0.0`), Description: "Fraction of the cycle token budget reserved for the entity-dedup phase (0.0-1.0). SQL-only phase; default 0.0 means no per-phase slice — the phase shares the root budget and runs whenever the cycle has remaining tokens.", Category: "dreaming_phase_budget", Min: ptrF(0), Max: ptrF(1), Step: ptrF(0.05)},
	{Key: service.SettingDreamEmbeddingBackfillFraction, Type: "number", DefaultValue: json.RawMessage(`0.10`), Description: "Fraction of the cycle token budget reserved for the embedding-backfill phase (0.0-1.0). Used by the embedder when re-embedding rows whose vectors are missing.", Category: "dreaming_phase_budget", Min: ptrF(0), Max: ptrF(1), Step: ptrF(0.05)},
	{Key: service.SettingDreamParaphraseFraction, Type: "number", DefaultValue: json.RawMessage(`0.05`), Description: "Fraction of the cycle token budget reserved for the paraphrase-dedup sweep (0.0-1.0). Used by the embedder when probing nearest-neighbours.", Category: "dreaming_phase_budget", Min: ptrF(0), Max: ptrF(1), Step: ptrF(0.05)},
	{Key: service.SettingDreamTransitiveFraction, Type: "number", DefaultValue: json.RawMessage(`0.0`), Description: "Fraction of the cycle token budget reserved for the transitive relationship-discovery phase (0.0-1.0). SQL-only phase; default 0.0 means no per-phase slice.", Category: "dreaming_phase_budget", Min: ptrF(0), Max: ptrF(1), Step: ptrF(0.05)},
	{Key: service.SettingDreamContradictionFraction, Type: "number", DefaultValue: json.RawMessage(`0.40`), Description: "Fraction of the cycle token budget reserved for the contradiction-detection phase (0.0-1.0). Caps how many tokens the LLM-judge pair walk can consume so consolidation isn't starved.", Category: "dreaming_phase_budget", Min: ptrF(0), Max: ptrF(1), Step: ptrF(0.05)},
	...
	{Key: service.SettingDreamWeightAdjustFraction, Type: "number", DefaultValue: json.RawMessage(`0.0`), Description: "Fraction of the cycle token budget reserved for the weight-adjustment phase (0.0-1.0). SQL-only phase; default 0.0 means no per-phase slice.", Category: "dreaming_phase_budget", Min: ptrF(0), Max: ptrF(1), Step: ptrF(0.05)},
	...
	{Key: "recall.fusion.vector_weight", Type: "number", DefaultValue: json.RawMessage(`0.70`), Description: "Weight on each vector channel's RRF contribution (0.0-1.0). Together with lexical_weight, controls the relative pull of dense embedding vs sparse keyword evidence.", Category: "recall_fusion", Min: ptrF(0), Max: ptrF(1), Step: ptrF(0.05)},
	{Key: "recall.fusion.lexical_weight", Type: "number", DefaultValue: json.RawMessage(`0.30`), Description: "Weight on each lexical channel's RRF contribution (0.0-1.0). Raise to bias recall toward exact-token matches (entity names, version strings).", Category: "recall_fusion", Min: ptrF(0), Max: ptrF(1), Step: ptrF(0.05)},
	...
	{Key: service.SettingLifecycleSweepIntervalSeconds, Type: "number", DefaultValue: json.RawMessage(`300`), Description: "How often the lifecycle sweep runs (time-to-live expiry plus soft-delete purge), in seconds. Read once at startup.", Category: "lifecycle", RequiresRestart: true, Min: ptrF(10), Max: ptrF(3600), Step: ptrF(10)},
	{Key: service.SettingLifecycleBatchSize, Type: "number", DefaultValue: json.RawMessage(`100`), Description: "Maximum memories processed per lifecycle sweep pass. Hot-reloads on the next sweep.", Category: "lifecycle", Min: ptrF(1), Max: ptrF(10000), Step: ptrF(10)},
