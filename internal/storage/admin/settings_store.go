...
var settingsSchemas = []api.SettingSchema{
	...
	{Key: service.SettingMultiVectorEnabled, Type: "boolean", DefaultValue: json.RawMessage(`false`), Description: "Split each memory into topic facets (plus the whole-memory vector) so a query about one sub-topic of a multi-topic memory still retrieves it strongly. Off by default; enabling it adds per-memory sentence-embedding cost. After turning it on, use the Backfill button to facet memories stored beforehand.", Category: "enrichment_multi_vector"},
