...
var settingsSchemas = []api.SettingSchema{
	...
	{Key: service.SettingGraphChargeStrength, Type: "number", DefaultValue: json.RawMessage(`-15`), Description: "Default spacing force in the 3D graph layout (stored as a negative number; the graph page shows it as a positive 'repulsion' slider). More repulsion spreads nodes apart. Can be overridden per project.", Category: "graph_visualization", Min: ptrF(-100), Max: ptrF(0), Step: ptrF(1)},
	{Key: service.SettingGraphLinkDistance, Type: "number", DefaultValue: json.RawMessage(`15`), Description: "Default length of the links between connected nodes in the 3D graph layout. Lower pulls connected nodes closer; higher spreads the graph out. Can be overridden per project.", Category: "graph_visualization", Min: ptrF(5), Max: ptrF(100), Step: ptrF(1)},
