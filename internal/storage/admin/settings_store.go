...
var settingsSchemas = []api.SettingSchema{
	...
	{Key: service.SettingMultiVectorMaxFacets, Type: "number", DefaultValue: json.RawMessage(`8`), Description: "Maximum vectors stored per memory, counting the whole-memory facet plus topic facets. Caps storage and per-query work for long, many-topic memories.", Category: "enrichment_multi_vector", Min: ptrF(1), Max: ptrF(32), Step: ptrF(1)},
	...
	{Key: service.SettingRerankEnabled, Type: "boolean", DefaultValue: json.RawMessage(`false`), Description: "Re-score the top recall candidates with a cross-encoder (or LLM judge) reranker before the final result is returned, reordering by model-judged query relevance. Off by default and requires the Reranker provider slot to be configured. Also raise Rerank weight above 0 for it to affect the order.", Category: "ranking"},
	{Key: service.SettingRankWeightRerank, Type: "number", DefaultValue: json.RawMessage(`0`), Description: "How much the reranker's relevance score counts toward recall rank (0.0 to 1.0), added on top of the composite score rather than replacing similarity. Default 0, so the reranker has no effect until you raise it. Only used when Reranking is enabled and a Reranker slot is configured.", Category: "ranking", Min: ptrF(0), Max: ptrF(1), Step: ptrF(0.05)},
