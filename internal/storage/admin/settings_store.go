...
var settingsSchemas = []api.SettingSchema{
	...
	{Key: service.SettingEnrichmentWorkerPreEmbedConcurrency, Type: "number", DefaultValue: json.RawMessage(`1`), Description: "How many fact and entity model calls a worker makes in parallel within a batch. 1 (the default) runs them one at a time, safe for a single local GPU. Raise it only if your provider has spare capacity; parallel calls to a one-GPU Ollama backend queue up and can look like the system is stuck.", Category: "enrichment_performance", Min: ptrF(1), Max: ptrF(64), Step: ptrF(1)},
	...
	{Key: service.SettingFactExtractionRepeatPenalty, Type: "number", DefaultValue: json.RawMessage(`1.15`), Description: "Discourages the fact-extraction model from repeating itself (Ollama only; ignored by OpenAI-style providers). 1.0 means no penalty; 1.10 to 1.20 curbs runaway repetition on small models without distorting good output.", Category: "enrichment_performance", Min: ptrF(0.5), Max: ptrF(2), Step: ptrF(0.05)},
	{Key: service.SettingEntityExtractionRepeatPenalty, Type: "number", DefaultValue: json.RawMessage(`1.15`), Description: "Discourages the entity-extraction model from repeating itself (Ollama only). Same meaning as the fact-extraction repeat penalty.", Category: "enrichment_performance", Min: ptrF(0.5), Max: ptrF(2), Step: ptrF(0.05)},
	{Key: service.SettingFactExtractionTopK, Type: "number", DefaultValue: json.RawMessage(`0`), Description: "Top-K sampling for the fact-extraction call (Ollama only). 0 leaves it unset; typical values are 20 to 40.", Category: "enrichment_performance", Min: ptrF(0), Max: ptrF(200), Step: ptrF(1)},
	{Key: service.SettingEntityExtractionTopK, Type: "number", DefaultValue: json.RawMessage(`0`), Description: "Top-K sampling for the entity-extraction call (Ollama only). 0 leaves it unset; typical values are 20 to 40.", Category: "enrichment_performance", Min: ptrF(0), Max: ptrF(200), Step: ptrF(1)},
	{Key: service.SettingFactExtractionMinP, Type: "number", DefaultValue: json.RawMessage(`0`), Description: "Minimum-probability sampling cutoff for the fact-extraction call (Ollama only). 0 leaves it unset; typical values are 0.05 to 0.10.", Category: "enrichment_performance", Min: ptrF(0), Max: ptrF(1), Step: ptrF(0.01)},
	{Key: service.SettingEntityExtractionMinP, Type: "number", DefaultValue: json.RawMessage(`0`), Description: "Minimum-probability sampling cutoff for the entity-extraction call (Ollama only). 0 leaves it unset; typical values are 0.05 to 0.10.", Category: "enrichment_performance", Min: ptrF(0), Max: ptrF(1), Step: ptrF(0.01)},
	...
	{Key: service.SettingLifecycleOrphanGraceSeconds, Type: "number", DefaultValue: json.RawMessage(`3600`), Description: "How old an entity must be before it can be deleted as an orphan, in seconds. The grace period protects entities created mid-enrichment, before their relationships are written.", Category: "lifecycle", Min: ptrF(60), Max: ptrF(86400), Step: ptrF(60)},
	...
	// Provider prompt delivery and Ollama keep-warm controls.
	...
	{Key: service.SettingProviderOllamaKeepAlive, Type: "string", DefaultValue: json.RawMessage(`"5m"`), Description: "Ollama keep_alive value applied to chat and embedding calls so the model stays resident between requests (e.g. \"5m\", \"30s\", \"-1\" to keep it loaded indefinitely, \"0\" to unload immediately). Only applied to Ollama providers; ignored by strict OpenAI-compatible endpoints.", Category: "provider_ollama"},
	{Key: service.SettingProviderOllamaNumCtx, Type: "number", DefaultValue: json.RawMessage(`0`), Description: "Override Ollama's context window (num_ctx) per request. 0 leaves it to the model's Modelfile default. Only applied to Ollama providers.", Category: "provider_ollama", Min: ptrF(0), Max: ptrF(1048576), Step: ptrF(512)},
	...
	{Key: service.SettingDreamConsolidationClusterOverlapThreshold, Type: "number", DefaultValue: json.RawMessage(`0.3`), Description: "How much wording two memories must share to be grouped together for consolidation (0.0 to 1.0). Higher values make smaller, tighter groups.", Category: "dreaming_consolidation", Min: ptrF(0), Max: ptrF(1), Step: ptrF(0.05)},
