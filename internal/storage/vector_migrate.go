...
func streamPgVectors(ctx context.Context, db DB, kind VectorKind, dim int, fn func(VectorUpsertItem) error) (int, error) {
	...
	// Memory tables are faceted; migrate only facet 0 (the pooled whole-memory
	// vector). Topic facets are regenerated on the destination by the
	// multi-vector backfill after a backend switch, so copying them here would
	// only produce duplicate-key writes against the facet-0 point id.
	if spec.faceted {
		if where == "" {
			where = " WHERE v.facet_id = 0"
		} else {
			where += " AND v.facet_id = 0"
		}
	}
	query := fmt.Sprintf(
...
func streamSQLiteVectors(ctx context.Context, db DB, kind VectorKind, dim int, fn func(VectorUpsertItem) error) (int, error) {
	...
	var query string
	if soft {
		// soft == memory kind, which is faceted; migrate only facet 0 (topic
		// facets are regenerated by the multi-vector backfill post-switch).
		query = fmt.Sprintf(
			`SELECT v.%s, v.namespace_id, v.embedding FROM %s v JOIN %s p ON v.%s = p.id WHERE p.deleted_at IS NULL AND v.dimension = ? AND v.facet_id = 0`,
