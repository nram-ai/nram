...
ALTER TABLE memories ADD COLUMN faceted_at TEXT;
ALTER TABLE memories ADD COLUMN facet_count INTEGER;

-- Index predicate matches the WHERE in
-- MemoryRepo.ListMultiVectorBackfillCandidates exactly. Diverging the two
-- leaves SQLite unable to recognize the partial index as usable and forces a
-- full-table scan on every backfill candidate check.
