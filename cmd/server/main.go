...
func main() {
	...
	dreamRunner := dreaming.NewRunner(
		...
		// Uncovered backfill enqueues a FULL enrichment job for every live memory
		// that has NEVER been enriched (durable memories.enriched = false:
		// enrichment disabled at creation, a failed write-time enqueue, a direct
		// import/migration) — the unconditional safety net replacing the removed
		// NRAM_ENABLE_ENRICHMENT_BACKFILL boot hook, gated only on enrichment.enabled.
		// Coverage is keyed on the durable enriched flag, not on enrichment_queue
		// rows (which are cleared by the admin clear-completed endpoint and by
		// memory/namespace deletion), so a completed memory is never re-enqueued and
		// the steady state is a no-op. Unlike embedding backfill (which only
		// re-embeds), the full job restores extracted facts, entities, and
		// relationships. Runs before the narrower backfill phases; a memory it
		// enqueues is skipped by them via the partial unique index
		// idx_enrichment_queue_pending_memory. Enqueue-only, no LLM, so it consumes
		// no dream token budget and is deliberately absent from runner's
