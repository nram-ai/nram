...
**Memory and recall.** Hybrid retrieval fuses vector and lexical search (FTS5 on SQLite, `tsvector` on Postgres) with Reciprocal Rank Fusion, boosted by the knowledge graph and scored by six tunable terms (similarity, recency, importance, frequency, graph relevance, confidence). MMR reranking demotes near-duplicate results so recall stays diverse. Embedding runs off the write path, so stores stay fast. Semantic search backs onto pgvector, a pure-Go HNSW index, or Qdrant. Optional multi-vector facets split a multi-topic memory into per-topic vectors (plus the whole-memory vector) and score against the best-matching facet, so a query about one sub-topic retrieves the memory at that sub-topic's strength instead of a diluted average; off by default, with a backfill to facet existing memories.
